...
class LossSwin:
    ...
    def __call__(self, outputs1, outputs2, targets1, targets2):
        ...
        mae = self.criterion2(outputs2, targets2)
        print('MAELoss:{:.12f}'.format(mae.item()))

        #ssim = self.ssim(outputs2, targets2)
        #print('SSIM:{:.12f}'.format(ssim.item()))

        criterion = (self.weights[0] * mse + self.weights[1] * mae)
        #criterion = mae
